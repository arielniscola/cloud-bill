    ...
    ? createPortal(
        ...
        >
          ...
          <div className="px-3 py-2 border-b border-gray-100">
            <div className="flex items-center gap-2 px-2 py-1.5 rounded-lg bg-gray-50 border border-gray-200 focus-within:border-indigo-400 focus-within:ring-2 focus-within:ring-indigo-500/20 transition-all">
          ...
          {clearLabel && (
            <button
              ...
              onMouseDown={(e) => e.preventDefault()}
              onClick={handleClear}
              className={`w-full flex items-center gap-2 px-3 py-2.5 text-sm border-b border-gray-100 transition-colors ${
                ...
                return (
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm font-medium truncate leading-tight ${isSelected ? 'text-indigo-700' : 'text-gray-800'}`}>
                        {c.name}
                      </p>
                      {c.taxId && (
        ...
        <button
          ...
          className={`w-full flex items-center gap-2 px-3 py-2 text-sm rounded-lg border text-left transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 ${
            error
              ...
              : 'border-gray-200 bg-white focus:border-indigo-400 hover:border-gray-300'
        ...
        >
          {selected ? (
            <>
              ...
              <div className="flex-1 min-w-0">
                <span className="block truncate text-gray-900 font-medium text-sm leading-tight">
                  {selected.name}
                </span>
                {selected.taxId && (
                  <span className="block text-xs text-gray-400 truncate leading-none">
                    {selected.taxId}
                  </span>
                )}
              </div>
            ...
            <>
              ...
              <span className="flex-1 text-gray-400">{clearLabel ?? placeholder}</span>
              <ChevronDown className={`w-3.5 h-3.5 text-gray-300 flex-shrink-0 transition-transform duration-150 ${isOpen ? 'rotate-180' : ''}`} />
            </>
          )}
        </button>
