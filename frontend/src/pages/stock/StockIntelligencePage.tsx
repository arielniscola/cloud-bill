...
const ROW_CLASS: Record<StockRiskLevel, string> = {
  critical: 'bg-red-50/60 hover:bg-red-100/50',
  warning:  'bg-amber-50/40 hover:bg-amber-100/40',
  ok:       'hover:bg-gray-50/70',
  no_data:  'hover:bg-gray-50/70',
};

// ── Sub-components ─────────────────────────────────────────────────
function DaysCell({ days, risk }: { days: number | null; risk: StockRiskLevel }) {
  if (days === null) return <span className="text-gray-400 text-xs">Sin ventas</span>;
  const cls =
    risk === 'critical' ? 'text-red-600 font-semibold' :
    risk === 'warning'  ? 'text-amber-600 font-semibold' :
    'text-gray-700';
...
function StatCard({
  ...
  return (
    <Card className={highlight ? 'ring-1 ring-red-200' : ''}>
    ...
    <tr className="border-b border-gray-100 animate-pulse">
      {Array.from({ length: 9 }).map((_, i) => (
        <td key={i} className="px-4 py-3.5">
          <div className="h-3 bg-gray-100 rounded w-full max-w-[80px]" />
        </td>
      ))}
    </tr>
  );
...
function InsightRow({ item }: { item: StockInsight }) {
  return (
    <tr className={`border-b border-gray-100 transition-colors ${
      item.isDeadStock ? 'bg-purple-50/40 hover:bg-purple-100/30' : ROW_CLASS[item.riskLevel]
    }`}>
      ...
      <td className="px-4 py-3 text-gray-600 whitespace-nowrap text-sm">{item.warehouseName}</td>
      <td className="px-4 py-3 text-right whitespace-nowrap">
        <span className="font-medium text-gray-800">{formatNumber(item.availableStock, 0)}</span>
        {item.minQuantity !== null && (
          <span className="text-gray-400 text-xs ml-1">/ mín {formatNumber(item.minQuantity, 0)}</span>
        )}
      </td>
      ...
      <td className="px-4 py-3 text-right whitespace-nowrap">
        {item.recommendedQty > 0
          ? <span className="font-medium text-violet-700">{formatNumber(item.recommendedQty, 0)}</span>
          : <span className="text-gray-400 text-xs">—</span>
        }
      </td>
      <td className="px-4 py-3 text-right whitespace-nowrap text-gray-700 text-sm">
        {item.estimatedCost > 0
          ? formatCurrency(item.estimatedCost, 'ARS')
          : <span className="text-gray-400 text-xs">—</span>
        }
      </td>
      <td className="px-4 py-3 text-right whitespace-nowrap text-gray-600 text-sm">
...
export default function StockIntelligencePage() {
  ...
  const tabs: { id: RiskTab; label: string; count: number; activeClass: string }[] = [
    { id: 'all',      label: 'Todos',        count: summary?.insights.length ?? 0, activeClass: 'text-gray-700 border-gray-400 bg-gray-100' },
    { id: 'critical', label: 'Críticos',     count: summary?.criticalCount ?? 0,   activeClass: 'text-red-700 border-red-400 bg-red-100' },
    { id: 'warning',  label: 'Alertas',      count: summary?.warningCount ?? 0,    activeClass: 'text-amber-700 border-amber-400 bg-amber-100' },
    { id: 'ok',       label: 'OK',           count: okCount,                        activeClass: 'text-emerald-700 border-emerald-400 bg-emerald-100' },
    { id: 'no_data',  label: 'Sin datos',    count: noDataCount,                   activeClass: 'text-gray-500 border-gray-300 bg-gray-50' },
    { id: 'dead',     label: 'Stock muerto', count: summary?.deadStockCount ?? 0,  activeClass: 'text-purple-700 border-purple-400 bg-purple-100' },
  ];
        ...
        <div className="px-4 pt-4 pb-3 border-b border-gray-100 space-y-2">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {tabs.map(t => (
              ...
              >
                {t.label}
                <span className="rounded-full px-1.5 py-0 text-[10px] font-bold leading-4 bg-white/60">
                  {t.count}
                </span>
              </button>
            ))}
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-8 pr-3 py-1.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-violet-300 w-full sm:w-64"
            />
          </div>
        </div>
          ...
          <table className="w-full text-sm min-w-[860px]">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/60">
                <th className="px-4 py-2.5 text-left text-xs font-semibold text-gray-500 whitespace-nowrap">Producto</th>
                <th className="px-4 py-2.5 text-left text-xs font-semibold text-gray-500 whitespace-nowrap">Almacén</th>
                <th className="px-4 py-2.5 text-right text-xs font-semibold text-gray-500 whitespace-nowrap">Disponible</th>
                <th className="px-4 py-2.5 text-right text-xs font-semibold text-gray-500 whitespace-nowrap">Ventas/día</th>
                <th className="px-4 py-2.5 text-right text-xs font-semibold text-gray-500 whitespace-nowrap">Días hasta 0</th>
                <th className="px-4 py-2.5 text-right text-xs font-semibold text-gray-500 whitespace-nowrap">Stock recom.</th>
                <th className="px-4 py-2.5 text-right text-xs font-semibold text-gray-500 whitespace-nowrap">Costo recom.</th>
                <th className="px-4 py-2.5 text-right text-xs font-semibold text-gray-500 whitespace-nowrap">Cap. inmov.</th>
                <th className="px-4 py-2.5 text-center text-xs font-semibold text-gray-500 whitespace-nowrap">Riesgo</th>
              </tr>
            </thead>
            <tbody>
              {isFirstLoad ? (
