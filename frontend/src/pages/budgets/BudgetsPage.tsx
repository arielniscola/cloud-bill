...
export default function BudgetsPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Presupuestos"
        subtitle={total > 0 ? `${total} presupuestos registrados` : undefined}
        actions={
          ...
          <div className="py-20 flex flex-col items-center text-center px-4">
            ...
            <p className="text-sm text-gray-400 mb-5 max-w-xs">
              {hasFilters
                ...
                : 'Creá tu primer presupuesto y empezá a gestionar tus ventas.'}
            </p>
            {!hasFilters && (
              <Button onClick={() => navigate('/budgets/new')}>
                <Plus className="w-4 h-4 mr-2" />
                ...
                <tbody className="bg-white divide-y divide-gray-100">
                  {budgets.map((b) => {
                    ...
                    return (
                      ...
                      >
                        <td className="px-4 py-3.5 text-sm font-semibold text-gray-900 tabular-nums">{b.number}</td>
                        <td className="px-4 py-3.5">
                          <span
                            title={INVOICE_TYPES[b.type]}
                        ...
                        <td className="px-4 py-3.5 text-sm text-gray-700">
                          {b.customer?.name ?? (
                            <span className="text-gray-400 italic text-xs">Consumidor final</span>
                          )}
                        </td>
                        <td className="px-4 py-3.5 text-sm text-gray-500 tabular-nums">{formatDate(b.date)}</td>
                        <td className="px-4 py-3.5 text-sm text-gray-500 tabular-nums">
                          {b.validUntil ? formatDate(b.validUntil) : <span className="text-gray-300">—</span>}
                        </td>
