...
function SubcategoryCard({
  ...
  return (
    <div className="group relative bg-gray-50 hover:bg-indigo-50/70 border border-gray-200 hover:border-indigo-200 rounded-xl p-4 transition-all duration-150 cursor-default select-none">
      <div className="flex items-start gap-2.5 pr-5 min-w-0">
        <FolderTree className="w-4 h-4 text-gray-400 group-hover:text-indigo-400 flex-shrink-0 mt-0.5 transition-colors duration-150" />
        <span className="text-sm font-medium text-gray-800 group-hover:text-gray-900 leading-snug break-words min-w-0">
          {category.name}
        </span>
      </div>
      ...
      <div className="absolute top-2.5 right-2.5 flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
        <button
          onClick={onEdit}
          title="Editar"
          className="p-1 rounded-md text-gray-400 hover:text-indigo-600 hover:bg-indigo-100 transition-[background-color,color] duration-150 active:scale-[0.95]"
        >
        ...
        <button
          onClick={onDelete}
          title="Eliminar"
          className="p-1 rounded-md text-gray-400 hover:text-red-500 hover:bg-red-100 transition-[background-color,color] duration-150 active:scale-[0.95]"
        >
    ...
    <div className="p-2 space-y-1 animate-pulse">
      {[80, 60, 90, 70, 50].map((w, i) => (
        <div key={i} className="flex items-center gap-2.5 px-3 py-2.5">
          ...
          <div className={`h-3.5 bg-gray-100 rounded flex-1`} style={{ maxWidth: `${w}%` }} />
          <div className="w-5 h-3.5 bg-gray-100 rounded-full flex-shrink-0" />
        </div>
      ))}
    </div>
  );
...
export default function CategoriesPage() {
  ...
  return (
    <div>
      ...
      {/* ── Two-panel card ── */}
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden flex min-h-[560px]">

        {/* ── Left: root category list ── */}
        <div className="w-56 xl:w-64 flex-shrink-0 border-r border-gray-100 flex flex-col">
                ...
                return (
                  ...
                  >
                    {isSelected
                      ...
                      : <Folder className="w-4 h-4 text-gray-400 flex-shrink-0 group-hover:text-gray-500 transition-colors duration-150" />
                    }
          ...
          <div className="p-3 border-t border-gray-100 flex-shrink-0">
            <button
              onClick={() => openModal()}
              className="flex items-center gap-2 text-sm font-medium text-indigo-600 hover:text-indigo-700 px-2 py-1.5 rounded-lg hover:bg-indigo-50 transition-[background-color,color] duration-150 active:scale-[0.98] w-full"
            >
              ...
              <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-4 flex-shrink-0">
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  <button
                    ...
                    className="flex items-center gap-1.5 text-sm font-medium text-gray-500 hover:text-gray-700 px-2.5 py-1.5 rounded-lg hover:bg-gray-100 transition-[background-color,color] duration-150 active:scale-[0.98]"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 text-sm font-medium text-indigo-600 hover:text-indigo-700 px-2.5 py-1.5 rounded-lg hover:bg-indigo-50 transition-[background-color,color] duration-150 active:scale-[0.98]"
                  >
                  ...
                  <button
                    ...
                    className="p-1.5 rounded-lg text-gray-300 hover:text-red-500 hover:bg-red-50 transition-[background-color,color] duration-150 active:scale-[0.98]"
                  >
              ...
              <div className="flex-1 p-6 overflow-y-auto [scrollbar-width:thin]">
                {subcategories.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-center py-10">
                    ...
                    <p className="text-sm font-semibold text-gray-700 mb-1">Sin subcategorías</p>
                    <p className="text-xs text-gray-400 mb-4 max-w-xs leading-relaxed">
                      ...
                      <span className="font-medium text-gray-600">{selectedRoot.name}</span>.
                    </p>
                    <button
                      onClick={() => openModal(undefined, selectedRoot.id)}
                      className="flex items-center gap-1.5 text-sm font-medium text-indigo-600 hover:text-indigo-700 px-3 py-2 rounded-lg hover:bg-indigo-50 transition-[background-color,color] duration-150 active:scale-[0.98]"
                    >
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-3">
                    ...
                    <button
                      onClick={() => openModal(undefined, selectedRoot.id)}
                      className="flex flex-col items-center justify-center gap-1.5 border-2 border-dashed border-gray-200 rounded-xl py-5 px-3 text-gray-400 hover:border-indigo-300 hover:text-indigo-500 hover:bg-indigo-50/50 transition-all duration-150 active:scale-[0.97]"
                    >
                      <Plus className="w-4 h-4" />
      ...
      >
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          {parentIdValue && (
            <div className="flex items-start gap-2 text-xs text-indigo-700 bg-indigo-50 border border-indigo-100 rounded-lg px-3 py-2.5">
